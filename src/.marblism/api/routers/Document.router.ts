...
export default function createRouter<Config extends BaseConfig>(router: RouterFactory<Config>, procedure: ProcBuilder<Config>) {
    return router({
        ...
        createMany: procedure.input($Schema.DocumentInputSchema.createMany).mutation(async ({ ctx, input }) => checkMutate(db(ctx).document.createMany(input as any))),
        ...
        deleteMany: procedure.input($Schema.DocumentInputSchema.deleteMany).mutation(async ({ ctx, input }) => checkMutate(db(ctx).document.deleteMany(input as any))),
